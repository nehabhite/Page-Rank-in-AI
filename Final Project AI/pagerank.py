...
def main():
    ...
    if len(sys.argv) != 2:
        sys.exit("Usage: python pagerank.py small/medium/large")
    corpus = crawl(sys.argv[1])
    # corpus = crawl("small")
    print(f"-------------Random Surfer Model------------")
    print(f"PageRank Results from Sampling (n = {SAMPLES})")
    s1 = time.time()
    ranks = sample_pagerank(corpus, DAMPING, SAMPLES)
    p_ranks = OrderedDict(sorted(ranks.items(), key=lambda x: x[1], reverse=True))
    s2 = time.time()
    for page in p_ranks:
        print(f"  {page}: ( Rank value-{ranks[page]:.4f} )")
    s3 = s2 - s1
    print(f"Total time taken: {s3:.4f} seconds")
    print(f"-------------Iterative Algorithm------------")
    s1 = time.time()
    ranks = iterate_pagerank(corpus, DAMPING)
    it_ranks = OrderedDict(sorted(ranks.items(), key=lambda x: x[1], reverse=True))
    s2 = time.time()
    s3 = s2 - s1
    print(f"PageRank Results from Iteration")
    for page in it_ranks:
        print(f"  {page}: ( Rank value-{ranks[page]:.4f} )")
    
    print(f"Total time taken: {s3:.4f} seconds")
    plt.plot(p_ranks.keys(), p_ranks.values())
    plt.title('Random Surfer Model')
    plt.show()

    plt.plot(it_ranks.keys(), it_ranks.values())
...
def iterate_pagerank(corpus, damping_factor):
    ...
    while max_rank_change > 0.001:
        ...
        new_ranks = {page: (rank / norm_factor)
    ...
    print('No. of Iterations required to converge- ', iterations)
